function X = NonCVX_MC_replica_version(y,M,sizeX,p,err,x_initial,normfac,insweep,tol,decfac)
...
f_current = norm(y-M(x,1)) + lambda*norm(x,p);%Suggested change 
while lambda > lambdaInit*tol
    ...
    for ins = 1:insweep
        ...
        w = abs(diag(S)).^(p-1);
        s = SoftTh(diag(S),w.*(lambda*p)/(2*alpha)); %%Suggested change 
        S = diag(s);
        X = U*S*V';
        x = X(:);
        f_current = norm(y-M(x,1)) + lambda*norm(x,p);  %Suggested change 
